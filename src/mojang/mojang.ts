...
const getUuid = memoize(fetchUuidFromAPI, {
	promise: true,
	maxAge: 24 * 60 * 60 * 1000
})
const getUsername = memoize(fetchUsernameFromAPI, {
	promise: true,
	maxAge: 24 * 60 * 60 * 1000
})
const getSkin = memoize(fetchSkinFromAPI, {
	promise: true,
	maxAge: 15 * 60 * 1000
})
...
async function fetchUuidFromAPI(
	...
	if (mojangResponse.ok)
		throw new MojangAPIError(`Invalid username.`, mojangResponse.status)
	throw new MojangAPIError(
		`Mojang API error ${mojangResponse.statusText}`,
		mojangResponse.status
	)
}
...
async function fetchSkinFromAPI(uuid: string): Promise<string> {
	...
	if (profileResponse.status !== 200) {
		throw new Error(
			`Mojang API returned code ${profileResponse.status} while getting profile data for ${uuid}.`
		)
	}
	const content = (await profileResponse.json()) as any
	const textureUrl = JSON.parse(
		Buffer.from(content.properties[0].value, "base64").toString("ascii")
	).textures.SKIN.url
	const textureResponse = await fetch(textureUrl)
	const buffer = new Uint8ClampedArray(await textureResponse.arrayBuffer())
	const helmet = await sharp(buffer)
