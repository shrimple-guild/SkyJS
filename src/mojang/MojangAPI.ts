import { MojangAPIError } from "../hypixel/errors/MojangAPIError.js"
import sharp from "sharp"

export class MojangAPI {
	async fetchMojangProfile(username: string): Promise<MojangProfileData | undefined> {
		const response = await fetch(`https://api.mojang.com/users/profiles/minecraft/${username}`)
		if (response.status == 200) {
			return response.json()
		} else if (response.status == 204 || response.status == 404) {
			return undefined
		} else {
			throw new MojangAPIError(
				`Error fetching Mojang profile: ${response.statusText}`,
				response.status
			)
		}
	}

	async fetchMojangSession(uuid: string): Promise<MojangSessionData | undefined> {
		const response = await fetch(
			`https://sessionserver.mojang.com/session/minecraft/profile/${uuid}`
		)
		if (response.status == 200) {
			return response.json()
		} else if (response.status == 204 || response.status == 404) {
			return undefined
		} else {
			throw new MojangAPIError(
				`Error fetching Mojang session: ${response.statusText}`,
				response.status
			)
		}
	}

	extractName(session: MojangSessionData) {
		return session.name
	}

	extractTextureData(session: MojangSessionData): MojangTextureData {
		const encodedTextureData = session?.properties[0].value
		return JSON.parse(Buffer.from(encodedTextureData, "base64").toString("ascii"))
	}

	async fetchSkin(textureData: MojangTextureData): Promise<Buffer> {
		if (textureData.textures.SKIN) {
			const textureResponse = await fetch(textureData.textures.SKIN?.url)
			return Buffer.from(await textureResponse.arrayBuffer())
		} else {
			return isSlim(textureData.profileId) ? MojangAPI.textureAlex : MojangAPI.textureSteve
		}
	}

	async fetchAvatar(skin: Buffer) {
		const helmet = await sharp(skin).extract({ left: 40, top: 8, width: 8, height: 8 }).toBuffer()
		const data = await sharp(skin)
			.extract({ left: 8, top: 8, width: 8, height: 8 })
			.composite([{ input: helmet }])
			.removeAlpha()
			.raw()
			.toBuffer()
		return data.toString("base64")
	}

	private static textureAlex = Buffer.from(
		"",
		"base64"
	)

	private static textureSteve = Buffer.from(
		"",
		"base64"
	)
}

function isSlim(uuid: string): boolean {
	return (
		1 ==
		(parseInt(uuid[7]!, 16) ^
			parseInt(uuid[15]!, 16) ^
			parseInt(uuid[23]!, 16) ^
			parseInt(uuid[31]!, 16))
	)
}

type MojangProfileData = {
	id: string
	name: string
}

type MojangSessionData = {
	id: string
	name: string
	properties: [
		{
			name: "textures"
			value: string
		}
	]
	profileActions: string[]
}

type MojangTextureData = {
	timestamp: number
	profileId: string
	profileName: string
	textures: {
		SKIN?: {
			url: string
			metadata?: {
				model: "slim"
			}
		}
		CAPE?: {
			url: string
		}
	}
}
