import { MojangAPIError } from "../hypixel/errors/MojangAPIError.js"
import sharp from "sharp"

export async function fetchMojangProfile(username: string): Promise<MojangProfileData | undefined> {
	const response = await fetch(`https://api.mojang.com/users/profiles/minecraft/${username}`)
	if (response.status == 200) {
		return response.json()
	} else if (response.status == 204 || response.status == 404) {
		return undefined
	} else {
		throw new MojangAPIError(
			`Error fetching Mojang profile: ${response.statusText}`,
			response.status
		)
	}
}

export async function fetchMojangSession(uuid: string): Promise<MojangSessionData | undefined> {
	const response = await fetch(`https://sessionserver.mojang.com/session/minecraft/profile/${uuid}`)
	if (response.status == 200) {
		return response.json()
	} else if (response.status == 204 || response.status == 404) {
		return undefined
	} else {
		throw new MojangAPIError(
			`Error fetching Mojang session: ${response.statusText}`,
			response.status
		)
	}
}

export async function fetchTextureData(session: MojangSessionData): Promise<MojangTextureData> {
	const encodedTextureData = session?.properties[0].value
	return JSON.parse(Buffer.from(encodedTextureData, "base64").toString("ascii"))
}

function isSlim(uuid: string): boolean {
	return (
		1 ==
		(parseInt(uuid[7], 16) ^
			parseInt(uuid[15], 16) ^
			parseInt(uuid[23], 16) ^
			parseInt(uuid[31], 16))
	)
}

export async function fetchSkin(textureData: MojangTextureData): Promise<Uint8ClampedArray> {
	if (textureData.textures.SKIN) {
		const textureResponse = await fetch(textureData.textures.SKIN?.url)
		return new Uint8ClampedArray(await textureResponse.arrayBuffer())
	} else {
		return isSlim(textureData.profileId) ? textureAlex : textureSteve
	}
}

export async function fetchAvatar(skin: Uint8ClampedArray) {
	const helmet = await sharp(skin).extract({ left: 40, top: 8, width: 8, height: 8 }).toBuffer()
	const data = await sharp(skin)
		.extract({ left: 8, top: 8, width: 8, height: 8 })
		.composite([{ input: helmet }])
		.removeAlpha()
		.raw()
		.toBuffer()
	return data.toString("base64")
}

const textureAlex = new Uint8ClampedArray(
	Buffer.from(
		"",
		"base64"
	)
)
const textureSteve = new Uint8ClampedArray(
	Buffer.from(
		"",
		"base64"
	)
)

type MojangProfileData = {
	id: string
	name: string
}

type MojangSessionData = {
	id: string
	name: string
	properties: [
		{
			name: "textures"
			value: string
		}
	]
	profileActions: string[]
}

type MojangTextureData = {
	timestamp: number
	profileId: string
	profileName: string
	textures: {
		SKIN?: {
			url: string
			metadata?: {
				model: "slim"
			}
		}
		CAPE?: {
			url: string
		}
	}
}
