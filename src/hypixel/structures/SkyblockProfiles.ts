import { APISkyblockProfile } from "../types/APIProfileTypes"
...
export class SkyblockProfiles {
  ...
  constructor(private player: PlayerInfo, profiles: APISkyblockProfile[]) {
    this.profiles = profiles.map(profile => new SkyblockProfile(this.player, profile))
  }
  ...
  get main() {
    return this.profiles.reduce((prev, cur) => (cur.member.skyblockLevel > prev.member.skyblockLevel ? cur : prev))
  }
