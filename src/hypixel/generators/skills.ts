import { skills } from "../../constants/leveling"
import { SkyblockProfile } from "../structures/SkyblockProfile"
...
export function generateSkills(member: APISkyblockMember) {
  ...
  return {
    farming: skills.farming.calculateLevel(member.experience_skill_farming ?? 0, farmingCap),
    ...
    foraging: skills.foraging.calculateLevel(member.experience_skill_foraging ?? 0),
    fishing: skills.fishing.calculateLevel(member.experience_skill_fishing ?? 0),
    enchanting: skills.enchanting.calculateLevel(member.experience_skill_enchanting ?? 0),
    alchemy: skills.alchemy.calculateLevel(member.experience_skill_alchemy ?? 0),
    taming: skills.taming.calculateLevel(member.experience_skill_taming ?? 0),
    carpentry: skills.carpentry.calculateLevel(member.experience_skill_carpentry ?? 0),
    runecrafting: skills.runecrafting.calculateLevel(member.experience_skill_runecrafting ?? 0),
