import { SkyblockTrophyFishData } from "../types/SkyblockProfileTypes"
import { APISkyblockMember, OptionalRecord } from "../types/APIProfileTypes"
...
export function generateTrophyFish(member: APISkyblockMember): SkyblockTrophyFishData {
  ...
  const { rewards, ...counts } = trophyFish
  const claimedTier = rewards.length != 0 ? tiers[Math.max(...rewards) - 1] : null
