import { dungeons } from "../../constants/leveling"
import { SkyblockMember } from "../structures/SkyblockMember"
import { SkyblockDungeonBossData, SkyblockDungeonFloorData } from "../types/SkyblockProfileTypes"
import { APIDungeonData, APISkyblockMember } from "../types/APIProfileTypes"
...
export function generateDungeons(member: APISkyblockMember) {
  ...
  return {
    level: dungeons.calculateLevel(member.dungeons?.dungeon_types?.catacombs?.experience ?? 0),
    classes: {
      healer: dungeons.calculateLevel(member.dungeons?.player_classes?.healer?.experience ?? 0),
      mage: dungeons.calculateLevel(member.dungeons?.player_classes?.mage?.experience ?? 0),
      archer: dungeons.calculateLevel(member.dungeons?.player_classes?.archer?.experience ?? 0),
      tank: dungeons.calculateLevel(member.dungeons?.player_classes?.tank?.experience ?? 0),
      berserk: dungeons.calculateLevel(member.dungeons?.player_classes?.berserk?.experience ?? 0),
    },
    floors: {
...
function generateBossData(member: APISkyblockMember, floor: number): number {
  return (
    (member.dungeons?.dungeon_types?.catacombs?.tier_completions?.[floor] ?? 0) +
    (member.dungeons?.dungeon_types?.master_catacombs?.tier_completions?.[floor] ?? 0)
  )
}

function generateFloorData(dungeon: APIDungeonData | undefined, floor: number): SkyblockDungeonFloorData {
  return {
